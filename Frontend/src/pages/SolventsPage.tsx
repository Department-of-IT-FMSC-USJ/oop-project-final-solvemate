...
export default function SolventsPage() {
    ...
    const envBadge = (score: string) => score === "LOW" ? "badge-green" : score === "MEDIUM" ? "badge-yellow" : "badge-red";

    const filtered = solvents.filter(s => {
        const matchSearch = s.name.toLowerCase().includes(search.toLowerCase()) || (s.chemicalFormula || "").toLowerCase().includes(search.toLowerCase());
        ...
        <div className="page-container">
            <div className="page-header">
                <div>
                    ...
                    <p className="page-subtitle">Manage solvent catalog with HSP, cost & regulatory data</p>
                </div>
            ...
            <div className="search-bar">
                ...
                <input placeholder="Search by name or formula..." value={search} onChange={e => setSearch(e.target.value)} />
                ...
                <span className="result-count">{filtered.length} solvent{filtered.length !== 1 ? "s" : ""}</span>
            </div>
            {loading ? <div className="loading-state">Loading solvents...</div> : filtered.length === 0 ? (
                <div className="empty-state"><p>◔</p><h3>No solvents found</h3><p>{isAdmin ? 'Click "Add Solvent" to get started.' : "No solvents available yet."}</p></div>
            ) : (
